...
export class AsistenciaService {
  ...
  async updateDetalle(id: number, detalle: { miembroId: number; presente?: boolean; justificado?: boolean; notas?: string }[]) {
    ...
    // Remove old detalle records
    await this.detalleRepo.delete({ asistenciaId: id });

    // Insert new detalle records
    if (detalle?.length) {
      const nuevos = detalle.map((d) =>
        this.detalleRepo.create({
          asistenciaId: id,
          miembroId: d.miembroId,
          presente: d.presente ?? true,
